...
class KDTree
{
  ...
  public:
    ...
    Point<Dim> findingNearestNeighbour(const Point<Dim>& query, KDTreeNode* current, int dimension) const;
  ...
  private:
    ...
    KDTreeNode *root;
    size_t size;
    void tree_creator(std::vector<Point<Dim>>& tree_points, int dimension, int left, int right, KDTreeNode*& root);
    void copy(KDTreeNode*& copy, KDTreeNode*& original);
