...
def transformations(Rab, Rbc, Tab, Tbc):
    ...
    Rac = Rab @ Rbc # Rotation matrix from c to a
    r = R.from_matrix(Rac) # Create a Rotation object from a rotation matrix.
    quat_ac = r.as_quat() # (x,y,z,w)
    euler_ac = r.as_euler('xyz', degrees=False) # Euler angles in 'xyz' order (roll, pitch, yaw)
    Tac = Tab + Rab @ Tbc # Translation of c with respect to a
...
class problem_set2:
    ...
    pid_w = PID(-2, -0.1, -0.06, setpoint=0.0, output_limits=(-5, 5))
    pid_v = PID(-1, -0.0, -0.0, setpoint=0.0, output_limits=(0, 4))
    ...
    def desired_trajectory(self, t):
        R = 10
        freq = 0.03
        self.xd = R * np.cos(2 * np.pi * freq * t)
        self.yd = R * np.sin(2 * np.pi * freq * t)
